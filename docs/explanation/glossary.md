...
| **dunning** (督促) | Professional payment reminders for overdue/unpaid invoices, with logged send history; **not** debt collection or legal enforcement. Sending is operator-controlled — either pressed by an operator, or run on a schedule the operator configured and can switch off | "collection", "chasing" |
...
| **scheduled dunning** | Unattended sending of the *same* notices under the *same* guards, on a per-organization schedule that is **off by default** (#400). It changes when a notice is sent, never what it says | "auto-collection"; treating it as a different kind of message |
